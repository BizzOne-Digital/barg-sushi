...
const MenuItemCard = ({ item }) => {
  ...
  const primaryName = language === "fr" && item.nameFr ? item.nameFr : item.name;
  const secondaryName = language === "fr" ? item.name : item.nameFr;
  const description = language === "fr" && item.descriptionFr ? item.descriptionFr : item.description;
  const pcsLabel = language === "fr" ? "mcx" : "pcs";
  ...
  return (
    <div className="menu-card">
      <div className="menu-card-img">
        {item.image?.url ? (
          <img src={item.image.url} alt={primaryName} loading="lazy" />
        ) : (
        ...
        <div className="menu-card-badges">
          {isSpicy && <span className="badge badge-red"><Flame size={10} /> {language === "fr" ? "Épicé" : "Spicy"}</span>}
          {isVeg && <span className="badge badge-veg"><Leaf size={10} /> {language === "fr" ? "Végé" : "Veg"}</span>}
          {item.featured && <span className="badge badge-gold"><Star size={10} /> {t(language, "menu_popular")}</span>}
      ...
      <div className="menu-card-body">
        <div className="menu-card-header">
          <h3 className="menu-card-name">{primaryName}</h3>
          {item.pieces && <span className="menu-card-pieces">{item.pieces} {pcsLabel}</span>}
        </div>

        {secondaryName && secondaryName !== primaryName && (
          <p className="menu-card-namefr">{secondaryName}</p>
        )}
